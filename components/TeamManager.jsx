...
export default function TeamManager({ teams, editingTeam, onSubmit, onCancelEdit, onDelete, onEdit, canEdit }) {
  ...
  const [logoPreview, setLogoPreview] = useState(null); // data-URL for instant preview
  ...
  useEffect(() => {
    if (editingTeam) {
      ...
      setLogoFile(null);
      setLogoPreview(editingTeam.logo_url || null); // show existing logo as preview
    } else {
      setName("");
      setLogoFile(null);
      setLogoPreview(null);
  ...
  async function handleSubmit(e) {
    ...
    if (!trimmed) return;
    setUploading(true);
    await onSubmit({ name: trimmed, logoFile });
    setUploading(false);
    setName("");
    setLogoFile(null);
    setLogoPreview(null);
        ...
        {canEdit ? (
          <form onSubmit={handleSubmit} className="space-y-3 max-w-xl">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="shrink-0 w-12 h-12 rounded-lg border border-neutral-600 bg-neutral-900 flex items-center justify-center overflow-hidden">
                {logoPreview ? (
              ...
              <div className="flex-1">
                <label className="block text-xs text-neutral-400 mb-1">Team Logo <span className="text-neutral-600">(PNG, JPG, SVG — optional)</span></label>
                <div className="flex items-center gap-2">
                  ...
                  {logoFile && (
                    <span className="text-xs text-neutral-500 truncate max-w-[140px]">{logoFile.name}</span>
                  )}
                </div>
              </div>
            </div>
