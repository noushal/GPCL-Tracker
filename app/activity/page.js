...
function ActivityLogInner() {
  ...
  // Sync debounced search → URL (skip on mount)
  ...
  useEffect(() => {
    if (sessionLoading || !isAuthed) { setLoading(false); return; }
    fetchPage();
    // eslint-disable-next-line react-hooks/exhaustive-deps
        ...
        <div className="bg-neutral-800/60 border border-neutral-700 rounded-2xl px-4 py-3 flex flex-col sm:flex-row gap-3 items-center justify-between">
          ...
          <div className="relative w-full sm:w-72">
            ...
            <input
              ...
              className="w-full bg-neutral-900 border border-neutral-600 rounded-lg pl-10 pr-9 py-2 text-sm text-white focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500"
            />
            {searchInput && (
              <button
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <div className="flex rounded-lg border border-neutral-700 overflow-hidden text-xs font-semibold">
              {ACTION_FILTERS.map(({ value, label }) => (
                <button
                  key={value}
                  onClick={() => pushParams({ action: value, page: 1 })}
...
// ─── Page shell — wraps inner in <Suspense> so useSearchParams is valid ───────
export default function ActivityPage() {
  return (
    <div className="min-h-screen p-4 md:p-8">
      <div className="max-w-[95%] mx-auto space-y-6">
        {/* Header lives outside Suspense so it always renders */}
